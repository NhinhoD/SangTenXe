...
import { COMPANY_PHONE, NAV_ITEMS } from '../constants';
    ...
    <div className="relative w-10 h-10 md:w-12 md:h-12 flex items-center justify-center">
      ...
      <div className="relative w-8 h-8 md:w-10 md:h-10 bg-gradient-to-br from-sky-500 to-blue-600 rounded-lg flex items-center justify-center shadow-lg transform group-hover:-translate-y-1 transition-transform">
        ...
        >
          ...
          <path d="M9 17h6" />
          <path d="m11 12 2 2 4-4" className="text-white" />
        </svg>
      </div>
    </div>
...
const Header: React.FC = () => {
  ...
  // Đóng menu và reset trạng thái mở rộng khi chuyển trang
  useEffect(() => {
    setIsMenuOpen(false);
      ...
      <div className="bg-slate-900 md:bg-white border-b border-gray-200 md:border-b-0">
        <div className="container mx-auto px-4">
          <div className="flex justify-between items-center h-12 md:h-14">
            ...
            <button 
              className="md:hidden flex items-center gap-2 text-white font-bold text-sm uppercase tracking-widest px-2"
              onClick={toggleMenu}
            >
            ...
            <nav className="hidden md:flex space-x-1 h-full">
              {NAV_ITEMS.map((item) => (
                <div key={item.path} className="relative group h-full flex items-center">
                  <Link
                    to={item.path}
                    className={`px-4 lg:px-6 h-full flex items-center gap-1.5 font-bold text-sm transition-all duration-200 border-b-4 ${
                      location.pathname === item.path || (item.path !== '/' && location.pathname.startsWith(item.path))
                        ? 'text-sky-600 border-sky-500 bg-sky-50/50' 
                        : 'text-slate-600 border-transparent hover:text-sky-600 hover:bg-slate-50'
                    }`}
                  >
                    {item.label}
                    ...
                    <div className="absolute top-full left-0 w-64 bg-white text-slate-700 shadow-2xl rounded-b-xl overflow-hidden opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50 transform origin-top-left border-t-4 border-sky-500 py-2">
                      {item.children.map((child) => (
                        <Link
                          key={child.path}
                          to={child.path}
                          className="block px-6 py-3 hover:bg-sky-50 border-l-4 border-transparent hover:border-sky-500 text-sm font-semibold transition-all hover:text-sky-600 hover:pl-8"
                        >
                          {child.label}
                        </Link>
                      ))}
            ...
            <div className="hidden md:block">
               <Link to="/lien-he" className="text-xs bg-sky-600 hover:bg-sky-700 text-white px-6 py-2.5 rounded-full font-black shadow-lg shadow-sky-200 transition-all hover:-translate-y-1 uppercase tracking-wider">
                 Gửi yêu cầu ngay
               </Link>
            </div>
            ...
            <div className="md:hidden">
                <a href={`tel:${COMPANY_PHONE.replace(/\s/g, '')}`} className="text-sky-400 font-bold text-xs uppercase tracking-tighter">
                    GỌI NGAY
                </a>
            </div>
          </div>
          ...
          <div className="md:hidden bg-white border-t border-gray-200 absolute w-full shadow-2xl z-50 overflow-y-auto max-h-[calc(100vh-120px)] animate-fade-up">
            {NAV_ITEMS.map((item) => {
              ...
              return (
                <div key={item.path} className="border-b border-gray-100 last:border-0">
                  <Link
                    to={item.path}
                    onClick={(e) => hasChildren ? handleMobileParentClick(item, e) : setIsMenuOpen(false)}
                    className={`flex items-center justify-between w-full px-6 py-4 font-bold text-slate-700 transition-all ${
                      isExpanded ? 'text-sky-600 bg-sky-50/50' : ''
                    }`}
                  >
                    ...
                    {hasChildren && (
                      <ChevronDown 
                        size={20} 
                        className={`transition-transform duration-300 ${isExpanded ? 'rotate-180 text-sky-600' : 'text-slate-400'}`} 
                      />
                    )}
                  </Link>
                  ...
                  {hasChildren && (
                    ...
                    >
                      <Link
                        ...
                        className="block px-10 py-3 text-sm font-bold text-sky-600 bg-sky-100/30 border-b border-gray-100"
                      >
                      ...
                      {item.children.map(child => (
                        <Link
                          ...
                          to={child.path}
                          onClick={() => setIsMenuOpen(false)}
                          className={`block px-10 py-3.5 text-sm text-slate-600 hover:text-sky-600 border-b border-gray-100 last:border-0 transition-colors ${
                            location.pathname === child.path ? 'text-sky-600 font-bold bg-white' : ''
                          }`}
                        >
                           • {child.label}
                        </Link>
            ...
            <div className="p-6 bg-gray-50 border-t border-gray-100">
               <Link 
                to="/lien-he" 
                onClick={() => setIsMenuOpen(false)} 
                className="block w-full text-center bg-gradient-to-r from-sky-500 to-blue-600 text-white py-4 rounded-xl font-black shadow-lg shadow-sky-200 uppercase tracking-widest text-sm"
               >
                 LIÊN HỆ NGAY
               </Link>
            </div>
